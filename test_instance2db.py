...
def create_shell(input_file, effective_filter):
    ...
    for inst in sorted_instances:
        ...
        for ref in inst.findall("ComponentInstanceReference"):
            ...
            if ref_silicons:
                ref_silicon_match = any(effective_filter in (s.text or "") for s in ref_silicons)
        ...
        if ref_elem is not None and ref_elem.text:
            ...
            for other_inst in root.findall(".//Instance"):
                ...
                if int_class_id is not None and int_class_id.text and int_class_id.text.strip() == ref_value:
...
def process_interfaces_with_ext_vlnv(ipdefs_file, fileref, data, efilter):
    if ipdefs_file is None:
        ...
        return None  
    root = ipdefs_file if isinstance(ipdefs_file, ET.Element) else ET.fromstring(ipdefs_file)
...
def get_myname_from_root(root):
    ...
    result = []
    if short_name is not None and short_name.text and len(re.sub(r'^.*?:(.*?)\"?$', r'\1', short_name.text)) > 0:
        clean_name = re.sub(r'^.*?:', '', short_name.text).replace('"', '')
        result.append(clean_name)
        result.append(clean_name + '_')
    elif name is None or (name is not None and (not name.text or not name.text.strip())):
        if ext_vlnv_name is not None and ext_vlnv_name.text:
            result.append(ext_vlnv_name.text.lower())
    elif name is not None and ext_vlnv_name is not None and name.text.strip() == ext_vlnv_name.text.lower():
        result.append(ext_vlnv_name.text.lower())
    else:
...
def process_filters(parameter_maps, filter_params, default_filters, shell, Ipdefs, fileref):
    result_root = []
    ...
    if default_filters is not None:
        for param in default_filters:
            ...
            tokens = cleaned_value.strip().split()
            for token in tokens:
                initial_value_node = ET.Element("InitialValue", {
                    "Name": name_text,
                    "Style": "Designation"
                })
    ...
    if Ipdefs is not None and fileref is not None:
        ...
        if parameters_el is not None:
            for param_decl in parameters_el.findall('./ParamDecl'):
                ...
                tokens = cleaned_value.strip().split()
                for token in tokens:
                    default_value_node = ET.Element("DefaultValue", {
                        "Name": name_text,
                        "Style": "Designation"
                    })
...
def create_vars(root_element, filter_params, addrs) -> list:#list of elements in variable vars
    collected_elements = []
...
def create_lines(vars_element=None, ip_defs=None, file_ref=None, filter_params=None, commons=None, shell=None):#create due vars .. lines
                ...
                for param_decl in params_node.findall('./ParamDecl'):
                    ...
                    if vars_element is not None:
                        ...
                        for var in vars_element.findall('.//*'):
                            ...
                            if var_name_elem is not None and var_name_elem.text == param_name:
                                name_exists = True
                                break
                    ...
                    type_value = ""
                    xsi_type = param_decl.get('{http://www.w3.org/2001/XMLSchema-instance}type')
                    ...
                    if shell is not None:
                        shell_param = shell.find(f"./ParameterMap[Name/text()='{param_name}']")
                        if shell_param is not None:
                            shell_value = shell_param.find('./Value')
                            if shell_value is not None:
                                ph_node = ET.SubElement(value_node, "ph")
                                ph_node.set("Type", "Inherit")
                                ph_node.text = shell_value.text if shell_value.text else ""
                    
                    if ph_node is None and commons is not None:
                        commons_path = f".//*[local-name()='Name' and text()='{param_name}']"
                        commons_name = commons.find(commons_path)
                        if commons_name is not None:
                            commons_value = commons_name.find('../*[local-name()="Value"]')
                            if commons_value is not None:
                                ph_node = ET.SubElement(value_node, "ph")
                                ph_node.set("Type", "Inherit")
                                ph_node.text = commons_value.text if commons_value.text else ""
                    
                    if ph_node is None:
                        default_value = param_decl.find('./DefaultValue')
                        ph_node = ET.SubElement(value_node, "ph")
                        ph_node.set("Type", "Default")
                        ph_node.text = default_value.text if default_value is not None and default_value.text else ""
...
def process_rows_to_parameters(lines_element):#iterate lines to rows and add smth into DB/instance element
    parameters = []
    ...
    rows.sort(key=lambda row: row.find('./Name').text if row.find('./Name') is not None and row.find('./Name').text else "")
...
def create_glines_element(gens=None, ip_defs=None, file_ref=None, commons=None, shell=None):#create glines from gens
    ...
    try:
        if gens is not None and len(gens) > 0:
            for gen in gens:
                ...
                name_text = name_elem.text if name_elem is not None and name_elem.text else ""
                name_node = ET.SubElement(row, "Name")
                name_node.text = name_text
                
                type_elem = gen.find('./Type')
                type_text = type_elem.text if type_elem is not None and type_elem.text else ""
                type_node = ET.SubElement(row, "Type")
                ...
                for generic_decl in params_node.findall('./GenericDecl'):
                    ...
                    if gens is not None:
                        ...
                        for gen in gens:
                            ...
                            if gen_name_elem is not None and gen_name_elem.text == param_name:
                                name_exists = True
                                break
                    ...
                    type_value = ""
                    xsi_type = generic_decl.get('{http://www.w3.org/2001/XMLSchema-instance}type')
                    ...
                    if shell is not None:
                        shell_gen = shell.find(f"./GenericMap[Name/text()='{param_name}']")
...
def rows_to_parameters(glines_element):#filter glines
    rows = sorted(glines_element.findall('./Row'), key=lambda row: (row.find('./Name').text or ''))
    ...
    for bus_map in instance.findall("BusInstanceReference/BusInterfaceMap"):
        ...
        if bus_type == "BusSlaveInterfaceMap" and interface is not None:
            ...
            if start_addr is not None and end_addr is not None:
                range_val = integerEssence(end_addr.text) - integerEssence(start_addr) + 1
                ...
                ET.SubElement(param_range, "Value").text = "0x" + decimal_to_hex(range_val)#add essence
                new_params.append(param_range)

        elif bus_type == "BusMasterInterfaceMap" and start_addr is not None and interfaces_container is not None:
            ...
            if master_id:
                for iface in interfaces_container.findall("Interface"):
                    target_id = iface.findtext("AddressBlock/XRefMasterInterface/XRefTargetID")
...
def collect_parameter_maps(inst_id, instance_root, ipdefs_root, effective_filter=None, visited=None):
    if visited is None:
        visited = set()
    ...
    for ref in instance_root.findall("ComponentInstanceReference"):
        ...
        if not silicons or (effective_filter and any(s in effective_filter for s in silicons)):
            for subref in ref.findall("ComponentInstanceRef"):
                ref_id = subref.text
                result_params.extend(
                    collect_parameter_maps(ref_id, instance_root, ipdefs_root, effective_filter, visited)
                )
...
def create_filter_element(instance, filter_params, raw):
    ...
    for pn in filter_params:
        ...
        parameter_maps = [p for p in raw if p.tag == "ParameterMap" and p.findtext("Name") == pn_str]
        ...
        if parameter_maps:
            ...
            param_decl = ET.Element("ParamDecl", {"xsi:type": "StringDecl", "Name": pn_str})
            ...
            param_decls = [p for p in raw if p.tag == "ParamDecl" and p.findtext("Name") == pn_str]
...
def create_raw_and_filter_element(group, key, common, IPdefs , filter_params,efilter, parameter_element1):
    ...
    for instance in group: 
        ...
        if isinstance(IPdefs, str):
            IPdefs = ET.fromstring(IPdefs)
        parameters = collect_parameter_maps(int_class_id, instance, IPdefs, efilter)#parammaps template
...
def create_parameter_element(instance, raw, filter_params):
    ...
    for param_decl in raw:
        if param_decl.tag == "ParamDecl" and param_decl.findtext("Name") not in filter_params:
            ...
            matching_param_map = [p for p in raw if p.tag == "ParameterMap" and p.findtext("Name") == pn]
    ...
    if root:
        ...
        for port in root.findall("./InterfaceDefPort"):
            ...
            if xref_elem is None or xref_elem.text is None:
                continue
            target_id = xref_elem.text

            signal = next(
                (
                    sign
                    for sign in signals
                    if (sign_id := sign.find("ID")) is not None
                    and sign_id.text
                    and target_id in sign_id.text
                ),
                None,
            )
            if signal is None:
            ...
            if port_id in excludes:
                ...
                continue
            if includes and port_id not in includes:
                add_comment("not including ", signal_keys, signal_values, port_keys, port_values, signal, port)
                continue

            if ("owner" in port_keys and "concept" not in port_values) or ("owner" in signal_keys and "concept" not in signal_values):
                continue
            ...
            elif port_name is not None and port_name.text:
                mn = re.sub(r"^(.*?)_a?[io]s*$", r"\1", port_name.text)
...
def get_interface_def_role(socket, role, my_name, iface):
    ...
    for view in interface_views:
        if view.find("Name") is not None and view.find("Name").text == "RTL":
            ...
            is_connected_value = is_connected.text if is_connected is not None else "false"
...
def to_boolean(input_value):
    if input_value and (isinstance(input_value, list) or isinstance(input_value, tuple)):
...
def main():
    try:
        Iproot = "C:/Users/Makarevich/Sync/Instance2db/parsed_context_spirit.xml"
        fixes_file = 'C:/Users/Makarevich/Sync/fixes/'
        toolversion = 1.4
        Doc_Author = 'Abc Abc'
        drive = "file:"
        disc = ""
        filter_params = ['audience', 'platform', 'product', 'package', 'props', 'otherprops']
        input = 'C:/Users/Makarevich/Sync/fixes/instance_sheet_TC49x.xml'
        efilter = find_filter(input_file='C:/Users/Makarevich/Sync/fixes/instance_sheet_TC49x.xml')
                    ...
                    if file:
                        ...
                        if os.path.exists(file) and os.path.isfile(file):
                            for key in keys:
                                ...
                                members = get_interface_def_role(file, role, myname, combined_root)
                                
                                instance_element.append(members)
                        else:
                            print(f"There is no {file}")

#            filters = process_filters(parameter_maps, filter_params, deault_filters, shell, xml_file, fileref)
            #filters = process_filters_grouped(filters)
#           addrs = process_bus_interfaces(file, xml_file)
...
#     lines = create_lines(vars, xml_file, key, filter_params, common, shell)
            ...
            '''
                ...
                xml_file = parammaps(file,key)#this is 1 ipdefs for 1 instance 
                #its a part of creation ParaMaps2!!!!


                #paramaps2
                parameter_element = transform_parameters(file)
                raw = create_raw_and_filter_element(group, key, common, xml_file , filter_params, efilter, parameter_element)#group -- list with instances
                #its a part of creation DB element 
                
                results = process_interfaces_with_ext_vlnv(xml_file, key, data, efilter)
                if results:
                    for path in results:
                        create_socket(path)
